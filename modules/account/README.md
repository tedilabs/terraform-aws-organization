...
| <a name="input_delegated_services"></a> [delegated\_services](#input\_delegated\_services) | (Optional) A list of service principals and their regional configurations for which you want to make the member account a delegated administrator. For regional services like Inspector2 and Macie, you can specify target regions. Empty regions means all regions. | <pre>list(object({<br/>    name    = string<br/>    regions = optional(set(string), [])<br/>  }))</pre> | `[]` | no |
