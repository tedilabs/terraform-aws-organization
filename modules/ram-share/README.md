...
| <a name="input_resources"></a> [resources](#input\_resources) | (Optional) A map of the Amazon Resource Names (ARNs) of the resource to associate with the RAM Resource Share. Each key is only used internally within Terraform code. Each value is the ARN of the resource. | `map(string)` | `{}` | no |
